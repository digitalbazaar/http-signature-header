...
function extractPseudoHeaders(params, requestOptions) {
  for(const key in Object.values(PSEUDO_HEADERS)) {
...
api.createSignatureString = ({includeHeaders, requestOptions} = {}) => {
  ...
  return _signingString({
    includeHeaders, headers, method, url, host,
...
function _signingString({
  includeHeaders, headers, method, url,
  ...
  for(const h of includeHeaders) {
    if(h === '(request-target)') {
      ...
      const pseudoHeader = PSEUDO_HEADERS[h.trim()];
      if(pseudoHeader) {
        addValue({
          result, validate, key: pseudoHeader,
