 ...
 * @param {boolean} [authHeader=true] - Is the Signature Header an Authorization
 *   Header.
 *
 * @returns {object} Parsed options.
 */
function parseSignatureHeader(sigString, authHeader = true) {
  ...
  // if we don't have an Authorization header then
  // skip straight to parsing State.Params
  let state = authHeader ? State.Authorization : State.Params;
...
api.parseRequest = (request, options = {}) => {
  ...
  const authHeader = authzHeaderName.trim() === 'authorization';
  ...
  const parsed = parseSignatureHeader(authz, authHeader);
  ...
  if(authHeader && (!parsed.scheme || parsed.scheme !== 'Signature')) {
