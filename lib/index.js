...
api.createSignatureInputString = ({signatureInput, httpMessage}) => {
  ...
  const encodeInnerList = i => {
    // FIXME: this function can be removed in the future
    // params for lists are not supported by structured-fields-values yet
    const params = Object.entries(i.params || {}).map(
      ([key, value]) => `;${key}="${value}"`);
    return `${encodeList(i.value)}${params.join('')}`;
  };
  // @signature-params
