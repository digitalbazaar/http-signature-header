...
const _getCoveredValues = ({coveredContent, headers}) => {
  ...
  for(const {value, params} of coveredContent) {
    ...
    let listIndex = params.prefix;
    if(typeof listIndex === 'string') {
      listIndex = Number.parseInt(params.prefix);
    }
    // if we find a prefix we need to find the value in the list
    if(!Number.isNaN(listIndex) && Number.isFinite(listIndex)) {
      // this can decode to a list or an inner list
      const _list = decodeList(headerFieldValue);
      ...
      coveredValues[value] = list[params.prefix].value;
      continue;
    }
  }
