...
function extractPseudoHeaders(params, requestOptions) {
  for(const key in Object.values(pseudoHeaders)) {
    if(params[key]) {
...
api.parseRequest = (request, options) => {
  ...
  if(parsed.params['key-id']) {
    parsed.param.keyId = parsed.params['key-id'];
  }
...
function _signingString({includeHeaders, headers, method, url,
  host, path, requestOptions, validate}) {
  const result = [];
  const addValue = (key, values, label) => {
  ...
  for(const h of includeHeaders) {
    if(h === '(request-target)') {
      ...
      const pseudoHeader = pseudoHeaders[h.trim()];
      if(pseudoHeader) {
        addValue(pseudoHeader, requestOptions, h);
      } else {
        addValue(h, headers);
      }
    }
