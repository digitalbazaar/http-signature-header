...
function extractPseudoHeaders(params, requestOptions) {
  const psuedoHeaders = ['created', 'expires', 'algorith', 'keyId', 'key-id'];
...
api.createSignatureString = ({includeHeaders, requestOptions} = {}) => {
  ...
  const {url, method = '', created, expires, keyId, algorithm} = requestOptions;
  const psuedoHeaders = {created, expires, keyId, algorithm};
  const {host, path} = _parseHostAndPath(url);
  return _signingString(
    {includeHeaders, headers, method, url, host, path,
      psuedoHeaders, validate: _validateSendHeader});
};
...
function _signingString(
  {includeHeaders, headers, method, url, host, path, psuedoHeaders, validate}) {
  const result = [];
