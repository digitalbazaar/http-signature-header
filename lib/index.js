...
api.createSignatureString = ({includeHeaders, requestOptions} = {}) => {
  ...
  const headers = {};
  for(const h of Object.keys(requestOptions.headers)) {
    headers[h.toLowerCase()] = requestOptions.headers[h];
  }
...
api.parseRequest = (request, options) => {
  ...
  parsed.signingString = _signingString(
    {includeHeaders, headers, method, url, validate: _validateReceiveHeader});
...
function _signingString(
  {includeHeaders, headers, method, url, host, path, validate}) {
  const result = [];
