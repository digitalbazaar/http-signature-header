...
export function parseRequest(request, options = {}) {
  ...
  const authzHeaderNameOptions = [options.authorizationHeaderName, 'signature', 'authorization']
  let authzHeaderName;
  for (const authzHeaderNameOption of authzHeaderNameOptions) {
    if(request.headers[authzHeaderNameOption]) {
      authzHeaderName = authzHeaderNameOption;
      break;
    }
  }
  if(!authzHeaderName) {
    throw new HttpSignatureError(
      'No header containing a signature present in the' + 
      'request using the provided options.',
      'SyntaxError'
    );
  }
