...
/** @namespace http-signature-header */
 ...
 * The Uint8Array should be the result of signing the signatureInput string.
 *
 * @param {object<string, Uint8Array>} signatures - An object with keys equal
 *  to the signature name & the signature as a Uint8Array.
 *
 * @returns {string} An FS encoded dictionary with  sigs as SF Byte sequences.
 * @see {@link https://datatracker.ietf.org/doc/html/rfc8941#section-3.2}
 * For dictionaries.
 * @see {@link https://datatracker.ietf.org/doc/html/rfc8941#section-3.3.5}
 * for Byte sequences.
*/
const createSignatureHeader = signatures => {
  return encodeDict(signatures);
};

/**
 * @function convertToSFInnerList
 * Converts coveredContent & params to SF Items.
 *
 * @param {object} options - Options to use.
 * @param {Array<string|Item>} options.coveredContent - An array of content ids.
 * @param {object} options.params - Parameters for the inner list itself.
 *
 * @returns {Item} SF Items for signing and string construction.
*/
const convertToSFInnerList = ({coveredContent, params}) => {
  ...
  return new Item(coveredContent.map(toSFItem), params);
};
 ...
 * For dictionaries.
*/
const createSignatureInputHeader = ({signatures, params}) => {
  const sigs = {};
  for(const [sig, sigInputs] of signatures) {
    ...
    sigs[sig] = convertToSFInnerList({coveredContent, params: _params});
  }
 ...
 *
 * Takes in either a Signature-Input header entry for verification or
 * a coveredContent array for signing and outputs a signing string.
 *
 * @param {object} options - Options to use.
 * @param {object} [options.signatureInput] - A structured field dictionary.
 * @param {Array<Item|string>} [options.coveredContent] - Content ids.
 * @param {object} [options.params] - Params for the coveredContent.
 * @param {object} options.httpMessage - A request or response message.
 *
 * @returns {string} The string to be signed.
*/
const createSignatureInputString = ({
  signatureInput,
  coveredContent,
  params,
  httpMessage}) => {
  ...
  if(signatureInput && coveredContent) {
    throw new HttpSignatureError(
      'You may only pass in "signatureInput" or "coveredContent"' +
       'not both.', 'SyntaxError');
  }
  // if you are not recreating a Signature Input string for verification
  // we need to get the signatureInput sf Item from the coveredContent & params
  if(!signatureInput) {
    signatureInput = convertToSFInnerList({coveredContent, params});
  }
  // @signature-params
  const sigParams = serializeInnerList(signatureInput);
  // the covered content should be a structured field list value
  const coveredValues = _getCoveredValues({
    coveredContent: signatureInput.value,
    headers: _headers
  });
  // this turns a structured list into a list of strings
  const includeHeaders = signatureInput.value.map(item => item.value);
  ...
  const paramsStrings = `"@signature-params": ${sigParams}`;
...
*/
const parseRequest = (request, options = {}) => {
  ...
  for(const signatureId in signatureInputs) {
    const signature = signatures[signatureId];
    const signatureInput = signatureInputs[signatureId];
    // if a signature does not have both a Binary Sequence
    // and the signature inputs to recreate it drop it
    if(!(signature && signatureInput)) {
      continue;
    }
    const entry = {
      errors: [],
      // a parsed sf inner list
      signatureInput,
